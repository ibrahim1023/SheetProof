...
def detect_formula_inconsistency_findings(inventory: list[FormulaRecord]) -> list[Finding]:
    ...
    for issue in detect_formula_consistency_issues(inventory):
        findings.append(
            Finding(
                ...
                risk_score=SEVERITY_SCORES["high"],
            )
        )
    return findings
...
def detect_hidden_external_dependency_findings(
    index: WorkbookIndex, inventory: list[FormulaRecord]
) -> list[Finding]:
    findings: list[Finding] = []
    hidden = set(index.hidden_sheets + index.very_hidden_sheets)

    for item in inventory:
        ref_hidden = [r for r in item.references if "!" in r and r.split("!", 1)[0].strip("'") in hidden]
        if ref_hidden:
            findings.append(
                Finding(
...
def detect_broken_reference_findings(inventory: list[FormulaRecord]) -> list[Finding]:
    findings: list[Finding] = []
    for item in inventory:
...
def detect_hardcoded_override_findings(index: WorkbookIndex) -> list[Finding]:
    findings: list[Finding] = []
...
def detect_volatile_formula_findings(inventory: list[FormulaRecord]) -> list[Finding]:
    findings: list[Finding] = []
    for item in inventory:
